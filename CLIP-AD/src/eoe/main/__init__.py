...
def default_argsparse(
    modify_descr: Callable[
        [
            str,
        ],
        str,
    ],
    modify_parser: Callable[[ArgumentParser], None] = None,
    ...
    parser.add_argument(
        "-wdk", "--weight-decay", type=float, default=1e-4, help="The weight decay."
    )
    parser.add_argument(
        "--milestones",
        type=int,
    ...
    args.ad_mode = {"ovr": "one_vs_rest", "loo": "leave_one_out", "ff": "fifty_fifty"}[
        args.ad_mode
    ]
...
def ms_argsparse(
    ...
    def combined_parser_modify(parser):
        parser.add_argument(
            ...
            nargs="+",
            choices=[
                "+".join((i, j)) for i, j in product(TRANSFORMS.keys(), DS_PARTS.keys())
            ],
    ...
    def combined_args_modify(args):
        args.ms_mode = [
            MSM(msm.split("+")[0], msm.split("+")[1], args.magnitude)
            for msm in args.ms_mode
        ]
        if modify_args is not None:
            modify_args(args)
...
def evolve_argsparse(
    ...
    def evolve_descr_modify(s: str):
        s += (
            "Repeats this complete procedure with different fixed Outlier Exposure (OE) subsets of the OE dataset. "
    ...
    def evolve_modify_parser(parser: ArgumentParser):
        parser.add_argument(
            "--ev-oesize", type=int, default=1, help="Determines the OE subset size."
        )
        parser.add_argument(
            "--ev-generation-pool",
            type=int,
    ...
    def evolve_modify_args(args):
        if args.load is not None:
            raise ValueError(
                "For the evolutionary algorithm, `--load` has no impact. Use `--ev-continue-run` instead. "
            )
        if args.oe_size != np.infty:
            raise ValueError(
                "For the evolutionary algorithm, `--oe-size` has no impact. Use `--ev-oesize` instead. "
            )
        if modify_args is not None:
            modify_args(args)
...
def create_trainer(
    ...
    oe_limit_samples: Union[int, List[int]] = np.infty,
    ...
    **kwargs,
    ...
    if continue_run is None:
        logger = Logger(
            pt.join(datapath, "results", superdir) if logpath is None else logpath,
            comment,
        )
    else:
...
def evolve_trainer(
    ...
    **kwargs,
) -> Tuple[ADTrainer, VisionDataset, Logger]:
    """
    ...
    evolve_dir = pt.join(
        kwargs.pop("superdir", "."),
        f"log_{time_format(time.time())}_evolve_{evolve_tag}",
    )
    continue_run = kwargs.pop("continue_run", None)
    ...
    ds = load_dataset(
        trainer.dsstr,
        trainer.datapath,
        trainer.get_nominal_classes(dummy),
        0,
        trainer.train_transform,
        trainer.test_transform,
        trainer.logger,
        trainer.oe_dsstr,
        trainer.oe_limit_samples,
        trainer.oe_limit_classes,
        trainer.msms,
    )
    if classes is not None and len(classes) == 1:
        ...
        raise NotImplementedError(
            "Atm, evolve for multiple classes at once does not work."
        )
...
def evolve_setup(
    ...
    args: Namespace,
    maxfit=True,
) -> Tuple[List[object], int, deap.base.Toolbox, dict, Tree]:
    """
    ...
    history = {
        ...
        "setup": {
            "oesize": oesize,
            "geneation_pool": generation_pool,
            "mutation_pool": mutation_pool,
            "mutation_indp": mutation_indp,
            "mutation_oneofkbest": mutation_oneofkbest,
            "mutation_chance": mutation_chance,
            "mate_chance": mate_chance,
            "generations": generations,
            "oeds": trainer.oe_dsstr,
            "select_toursize": select_toursize,
        },
    }
    ...
    toolbox.register(
        "evaluate", evaluate_individual, trainer=trainer, args=args, toolbox=toolbox
    )
    toolbox.register(
        "mate",
        mate_individuals,
        oeds=oeds,
        poolsize=mutation_pool,
        indp=mutation_indp,
        oneofkbest=mutation_oneofkbest,
    )
    toolbox.register(
        "mutate",
        mutate_individual,
...
def multiscale_experiment(
    args: Namespace,
    model: torch.nn.Module,
    train_transform: Compose,
    val_transform: Compose,
    magnitudes: List[int] = (0, 1, 2, 4, 8, 16, 32),
    **kwargs,
):
    """
    ...
    if len(continue_run) == 0:
        ...
        logger = Logger(
            args.continue_run if plot_elsewhere is None else plot_elsewhere, noname=True
        )
    magn0_models = None
    for i, magnitude in enumerate(magnitudes):
        if len(continue_run) > i:
            ...
            if all(
                [
                    msm.ds_part not in (TRAIN_NOMINAL_ID, TRAIN_OE_ID)
                    for msm in args.ms_mode
                ]
            ):
                raise NotImplementedError(
                    "For blur test only, the magn0 models have to be loaded from the snapshots!"
                )
            continue
        trainer = create_trainer(
            args.objective,
            ...
            train_transform,
            val_transform,
            msm=[msm.set_magnitude(magnitude) for msm in args.ms_mode],
            logpath=logger.dir,
            oe_limit_samples=args.oe_size,
            continue_run=continue_last_magnitude[1],
            **kwargs,
        )
        if magnitude != 0 and all(
            [msm.ds_part not in (TRAIN_NOMINAL_ID, TRAIN_OE_ID) for msm in args.ms_mode]
        ):
            trainer.epochs = 0
            _, results = trainer.run(args.classes, args.iterations, magn0_models)
        else:
            models, results = trainer.run(
                args.classes, args.iterations, continue_last_magnitude[0]
            )
    ...
    for s, (a, std) in zip(magnitudes, aucs):
        print(
            f"{args.ms_mode} with magnitude={s:02d} yielded {a*100:04.2f} +- {std*100:04.2f}."
        )
    results = {
        "magnitudes": magnitudes,
        "aucs": list(zip(*aucs))[0],
        "stds": list(zip(*aucs))[1],
        "classes": args.classes,
        "comment": args.comment,
        "ms_mode": [repr(msm) for msm in args.ms_mode],
...
def load_setup(
    path: str,
    args: Namespace,
    check_train_transform: Compose,
    check_val_transform: Compose,
    ...
    assert [
        x.replace("'normalize'", "normalize")
        for x in json.loads(json.dumps(check_train_transform, cls=SetupEncoder))
    ] == setup.pop(
        "train_transform"
    ), f"The loaded train transformation string representation does not match the set one. Please match manually. "
    assert [
        x.replace("'normalize'", "normalize")
        for x in json.loads(json.dumps(check_val_transform, cls=SetupEncoder))
    ] == setup.pop(
        "test_transform"
    ...
    assert setup_load is None or all(
        [isinstance(seed, str) or seed is None for cls in setup_load for seed in cls]
    )
    assert (
        setup.pop("dataset") == args.dataset
    ...
    assert (
        len(setup) == 0
    ), f"There are unexpected arguments in the loaded setup: {setup.keys()}."
    classes = (
        args.classes if args.classes is not None else range(no_classes(args.dataset))
    )
    snapshots = []
...
def load_evolve(
    ...
    check_train_transform: Compose,
    check_val_transform: Compose,
) -> Tuple[dict, List[object], int, Logger, deap.base.Toolbox]:
    """
    ...
    load_setup(
        pt.join(
            ...
            [
                ...
                if pt.isdir(pt.join(path, d)) and d.startswith("log_")
            ][0],
        ),
        args,
        check_train_transform,
        check_val_transform,
    )
    ...
    if len(history["pop"]) > 0:
        nodes = [
            [n for n in nodes if n.content.values == val][0].content
            for val in history["pop"][-1]
        ]
    else:
        raise ValueError(
            f"The loaded experiment at {path} has no finished generation that can be used to continue it."
        )
    pop = []
    for n in nodes:
    ...
    _, _, toolbox, _, _ = evolve_setup(
        args.ev_oesize,
        args.ev_generation_pool,
        args.ev_mutation_pool,
        args.ev_mutation_indp,
        args.ev_mutation_oneofkbest,
        args.ev_mutation_chance,
        args.ev_mate_chance,
        args.ev_generations,
        args.ev_select_toursize,
        trainer,
        dataset,
        args,
