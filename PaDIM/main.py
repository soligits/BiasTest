...
def parse_args():
    ...
    parser.add_argument(
        "--arch", type=str, choices=["resnet18", "wide_resnet50_2"], default="resnet18"
    )
...
def main():
    ...
    for class_idx in args.label:
        train_dataset.append(
            mvtec.MVTecDataset(
                args.data_path, class_name=mvtec.CLASS_NAMES[class_idx], is_train=True
            )
        )
    ...
    class_indices = get_label_str(args.label)

    print(f"Normal Classes: {class_name} - ({class_indices})")
    ...
    train_feature_filepath = os.path.join(
        args.save_path, "temp_%s" % args.arch, "train_%s.pkl" % class_name
    )
    if not os.path.exists(train_feature_filepath):
        ...
        for i in range(H * W):
            ...
            cov[:, :, i] = (
                np.cov(embedding_vectors[:, :, i].numpy(), rowvar=False) + 0.01 * I
            )
    ...
    for i in range(H * W):
        ...
        dist = [
            mahalanobis(sample[:, i], mean, conv_inv) for sample in embedding_vectors
        ]
